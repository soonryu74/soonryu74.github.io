...
def load(prefix):
    ...
    if not fs: raise SystemExit(f"{prefix} 자료 없음 — scripts/kosis_fetch_census.py 먼저 실행")
...
acc(load("DT_1PM2001"), lambda r, c: ("tot" if r["ITM_ID"] == "T10" else "low"), edu)
...
acc(load("DT_1PC2012"), job_key, job)
...
acc(load("DT_1PM2002"), mar_key, mar)
...
sgg_codes = [r["c"] for r in DS["regions"] if r["l"] == "sgg" and all(v in V.get(r["c"], {}) for v in VARS)]
print("변수별 커버리지:", {v: sum(1 for c in V if v in V[c]) for v in VARS}, "| 전 변수 확보 시군구", len(sgg_codes))
mu = {v: statistics.mean(V[c][v] for c in sgg_codes) for v in VARS}
sd = {v: statistics.pstdev(V[c][v] for c in sgg_codes) for v in VARS}
out = {"generated": datetime.date.today().isoformat(), "year": YEAR, "vars": [{"id": v, "label": LABEL[v], "mean": round(mu[v], 2), "sd": round(sd[v], 2)} for v in VARS],
       "method": "김동진 외(2013) 지역박탈지수 방식 재현: 변수별 전국 시군구 z점수 단순합(높을수록 박탈), 2020 인구주택총조사 집계표(20% 표본·등록센서스) 기반 근사. 자동차 미소유·낙후 주거시설은 자료가 없어 제외(7개 변수).", "regions": {}}
rows = []
for c in sgg_codes:
    z = {v: (V[c][v] - mu[v]) / sd[v] for v in VARS}; idx = sum(z.values())
    rows.append((c, idx, z))
rows.sort(key=lambda x: -x[1]); n = len(rows)
for i, (c, idx, z) in enumerate(rows):
    q = 5 - min(4, int(i / n * 5))          # 5 = 가장 박탈(상위 20%), 1 = 가장 덜 박탈
    out["regions"][c] = {"idx": round(idx, 2), "q": q, "rank": i + 1, "n": n, "z": {v: round(z[v], 2) for v in VARS}, "v": {v: round(V[c][v], 1) for v in VARS}}
# 시도: 시군구 인구가중 평균 지수(참고)
for s in [r for r in DS["regions"] if r["l"] == "sido"]:
    kids = [(r["c"], pop.get(r["c"], {}).get("tot", 0)) for r in DS["regions"] if r["l"] == "sgg" and r["p"] == s["c"] and r["c"] in out["regions"]]
    if kids and sum(p for _, p in kids):
        idx = sum(out["regions"][c]["idx"] * p for c, p in kids) / sum(p for _, p in kids)
        out["regions"][s["c"]] = {"idx": round(idx, 2), "q": None, "rank": None, "n": n, "sido_avg": True}
...
print("상위(박탈 큼):", [(name[c], round(i, 1)) for c, i, _ in rows[:5]]); print("하위(덜 박탈):", [(name[c], round(i, 1)) for c, i, _ in rows[-5:]])
print("SD", round(statistics.pstdev([i for _, i, _ in rows]), 2), "| 저장 data/deprivation.json")
