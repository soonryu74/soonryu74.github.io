...
tableSlide({
  ...
  rows: [
    ...
    ['자료', '지역사회건강조사 98 · e-지방지표 157', '지역사회건강조사 41(시군구 단위) + 사망·감염병·검진·자원·인구·환경 115 + 암검진 7 + 검진 2024 + 건강수명·박탈지수'],
    ['지역 단위', '시도·시군구', '시도 17 · 시군구 231 · 보건소 조사 단위 258(자료 유무 표기)'],
...
statsSlide({
  ...
  stats: [
    { n: '170+', l: '지표', d: '지역사회건강조사 41 · 결과·환경 DB 115 · 국가암검진 7 · 일반검진 4(신규) · 건강수명 3 · 박탈지수 1' },
    { n: '258', l: '보건소 조사 단위', d: '질병관리청 「2025 지역건강통계 한눈에 보기」 기준. 시도 17 · 시군구 231' },
...
tableSlide({
  ...
  head: ['자료원', '기관', '지표', '연도', '갱신'], colW: [3.6, 2.6, 1.0, 2.1, 3.03], fs: 10.5,
  rows: [
    ...
    ['건강보험·의료이용·일반검진', '국민건강보험공단', '40', '2008~2024', '연 1회'],
    ['인구·사회·경제', '국가데이터처·행정안전부', '20', '2008~2024', '연 1회'],
    ['환경·안전', '환경부·국토부·도로교통공단', '28', '2008~2024', '연 1회'],
    ['건강수명(근사 산출)', '자체 산출', '3', '2008~2024', '사망원인 공표 후'],
    ['지역박탈지수(근사)', '자체 산출(2020 총조사)', '1', '2020', '총조사 5년'],
...
cardsSlide({
  ...
  items: [
    ...
    { icon: '시군', h: '시군구 231개', b: '사망률·검진·인구 등 행정 통계의 기본 단위. 수원시처럼 일반구가 있는 시는 구 자료를 시로 합산했습니다.' },
...
shot({ t: '화면 구성', sub: '제목(누르면 첫 화면) · 메뉴 탭 · 선택 컨트롤 · 연도 · 요약 타일 · 카드들 · 하단 출처', img: ['A_home_top'],
  bullets: ['① 제목: 누르면 언제든 「지표 분석」 첫 화면으로 돌아갑니다.', '② 메뉴 탭 12개: 지표 분석 · 지역 프로파일 · 성과지표 · 지역 비교 · 예방·관리 · 연관지표 · 핫스팟 · 연대기 전시관 · 조사 단위 · 자료원 · 의견·문의', '③ A−/A+ 글자 크기, 다크 모드 전환', '④ 지표·시도·시군구·비교 담기·값 유형 컨트롤', '⑤ 연도 슬라이더와 ▶ 재생', '⑥ 요약 타일 4개와 분석 카드'],
...
shot({ t: '3.1 지표 분석 — 요약 타일', sub: '선택 지역 값 · 전국 시군구 중앙값 · 순위 · 백분위를 한 줄로', img: ['A_kpis'], wide: true,
  ...
  tips: ['보고서 첫 문장은 이 타일 네 개로 충분합니다: "우리 지역 비만율 33.1%로 전국 시군구 중앙값보다 3%p 높고, 231곳 중 180위".'] });
...
shot({ t: '3.1 지표 분석 — 단계구분도 ② 전국 시군구 · ③ 시도 내 시군구', img: ['B_map_nation'], img2: ['B_map_insido'],
  bullets: ['전국 시군구: 231개 시군구 전체 분포. 분위는 전국 기준.', '시도 내 시군구: 우리 시도만 확대, 분위도 시도 안에서 다시 계산.', '값 없는 지역은 회색.'],
...
shot({ t: '3.1 지표 분석 — 순위 ① 기본', sub: '집단 탭: 17개 시도 · 전국 시군구 · 시도 내 시군구 · 보건소 단위', img: ['B_analysis_02'], img2: ['B_rank_rev'],
  bullets: ['막대 길이 = 값. 선택 지역은 파란 막대, 같은 시도 지역은 옅게 표시.', '「양호한 순 ▼ / 나쁜 순 ▲」로 정렬을 뒤집습니다.', '40개 이하 목록은 한 줄로 전부 펼쳐지고, 전국 229개는 스크롤 상자(인쇄 시 전부 펼침).'],
...
shot({ t: '3.2 지역 프로파일 — 강점과 개선 과제', sub: '백분위 상위·하위 지표와 접힌 「개선 과제」 목록', img: ['C_profile_07'], img2: ['C_profile_08'],
  bullets: ['개선 TOP5는 하위 백분위 지표. 결과지표(사망·유병 4종)는 순위에서 제외되어 있습니다.', '「개선 과제」를 펼치면 하위 25% 지표 전체와 권고 사업(예방·관리 탭 연결)이 나옵니다.'],
...
shot({ t: '3.6 연관지표 — 두 지표의 관계', sub: '산점도 · 피어슨/스피어만/켄달 상관계수 · 연도별 상관', img: ['G_corr_01'], img2: ['G_corr_02'],
  bullets: ['X·Y 지표를 고르면 231개 시군구 산점도와 상관계수, 근사 p값이 나옵니다.', '우리 지역 점은 강조 표시.', '상관은 인과가 아닙니다. 고령화율처럼 둘 다에 영향을 주는 제3의 요인을 생각하십시오.'],
...
// 3.9 조사 단위
shot({ t: '3.9 조사 단위', sub: '보건소 조사 단위 258개와 보건지소·진료소 3,607곳 매핑', img: ['J_units_01'],
...
cautionSlide({ t: '해석 주의 ① 순위의 함정', items: [
  ...
  { h: '불안정값이 많은 지표는 순위를 쓰지 마십시오', b: '우울증상 유병률은 시군구의 62%가 불안정값입니다. 이런 지표는 시도 단위나 여러 해 평균으로만 말하십시오.' },
