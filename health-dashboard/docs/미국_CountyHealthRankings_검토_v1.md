...
| | CHR&R | 우리 대시보드 |
|---|---|---|
| 단위 | 미국 카운티 약 3,000개 | 시군구 229개 · 보건소 258개 |
...
우리 대시보드는 지금 **229개 시군구를 1위부터 229위까지 줄 세운다.** 「전체 보기」 막대 순위는 그 서열을 애니메이션으로 보여 주기까지 한다.
...
| 항목 | CHR&R | 우리 | 판정 |
|---|---|---|---|
| 서열 표기 | 10개 그룹(2024~) | **1~229위 순위 + 애니메이션** | ❌ 우리가 뒤처짐 |
...
| **2** | **「Health Group」 방식 병기** — 1~229위와 함께 10개 그룹(또는 5분위) 표기를 제공하고, 기본 표시를 그룹으로 전환 검토 | CHR&R 2024년 전환의 직접 이식 | 중 |
...
1번과 3번이 가장 급하다. **신뢰구간 없이 229위까지 줄 세우는 현재 화면이 가장 공격받기 쉬운 지점**이며, CHR&R가 같은 비판을 받고 결국 설계를 바꾼 바로 그 지점이기 때문이다.
...
> **정정(2026-09-24)**: 이 문서 초판은 시군구를 231개로 적었다. 제주시·서귀포시 안의 보건소 2곳(「제주」·「서귀포」)이 시군구로 잘못 분류되어 생긴 숫자였고, 바로잡은 기준은 229개(기초자치단체 226 + 제주 행정시 2 + 세종)다.
