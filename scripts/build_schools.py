#!/usr/bin/env python3
"""나이스 교육정보 개방포털(open.neis.go.kr) 학교기본정보 → ipsi/data/schools-neis-all.json (고교 상세는 KESS 기반 schools-hs.json 사용)

- NEIS_API_KEY 환경변수가 있으면 1,000건/페이지, 없으면 5건/페이지(무인증 샘플 모드)로 전 페이지를 순회한다.
- 초·중·고·특수·각종학교를 한 파일에 축약 필드로 저장(고교 페이지 검색용).
...
def main():
    ...
    print(f"완료: 전체 {len(mins)}개교, {time.time()-t0:.0f}초 → {OUT_MIN}")
