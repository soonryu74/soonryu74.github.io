...
def extract(text, date):
    ...
    for i, (pos, endpos, name, role) in enumerate(marks):
        ...
        nxt_is_mine = bool(nxt) and (nxt[2] + nxt[3]).startswith("식품의약품안전처")
        has_kw = any(k in q for k in MFDS_KW)
        ok, basis = agency_scope.decide(q, "mfds", nxt_is_mine, has_kw)
        if not ok:
            continue
        answer = ""
        ...
        while j < len(marks) and taken < 3:
            ...
            if (nname + nrole).startswith("식품의약품안전처"):
                ...
                break
        qc = agency_scope.excerpt(q, "mfds", MFDS_KW)   # 소관 근거가 보이도록 발췌
