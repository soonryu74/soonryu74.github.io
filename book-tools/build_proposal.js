...
c.push(P("이 소설은 자료만으로 쓰이지 않았다. 작중 \'독수리 오형제\'의 모델이 된 이들을 기관장으로, 교수로 만나 온 시간이 이야기의 바탕이 되었다. 서울 불광동에서 충청북도 오송으로 이어지는 스물두 해가 그대로 이 소설의 지리이자 연표다."));

c.push(H2("이 작품의 강점"));
[["모델이 있는 이야기", "작중 인물들의 모델이 된 이들을 오랜 시간 곁에서 보아 온 저자가 썼다. 실무의 결과 조직의 공기가 자료 조사만으로는 나오지 않는 밀도로 담겼다."],
 ...
 ["끝까지 쓴 저자", "에세이·동화·미니북 세 권을 펴낸 경험이 있고, 이번 작품은 1·2권 합계 1,250매를 완고로 마쳤다. 후속권 공백이 없다."],
...
// ─── 6. 저자 ───
c.push(H1("6. 저자 소개"));
c.push(P("서해원", { bold: true, size: 22, after: 180 }));
c.push(P("대학 시절 서예 동아리에서 붓을 처음 잡았다. 졸업 후에는 캘리그라피로 작은 작품들을 만들어 왔다. 글자를 손으로 새기는 일에서 시작해, 쓰고 그리는 창작을 이어 오고 있다."));
c.push(P("서예가 안동해 선생의 작품과 함께 엮은 에세이 『나는 날마다 오늘이 가장 좋다』, 동화 『금샘이와 마법의 옥구슬』, 미니북 『내 마음의 응급처방전』을 펴냈다."));
c.push(P("『역학조사관』은 첫 장편소설이다."));
c.push(H2("집필 계기"));
c.push(QUOTE("소설 속 \'독수리 오형제\'의 모델이 된 분들을 기관장으로, 교수로 만나 오면서 이 이야기를 쓰기 시작했다. 서울 불광동에서 충청북도 오송으로 이어지는 스물두 해를 기록으로 남기고 싶었다. 헌신하는 이들의 노고가 세상에 알려지기를 바라는 마음이다."));
c.push(P("이메일 · sonnet.storylab@gmail.com", { before: 240 }));
