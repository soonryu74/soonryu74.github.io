...
| 내 성적표 | `ipsi/my.html` | 성적 입력(원점수·평균·표준편차·등급·수강자 수) → 과목별 위치(정규분포 추정, 5등급제)·1등급 인원·강점 계열·2/3학년 과목 추천·목표별 조건 체크(세특 탐구 기록 반영)·자가 체크리스트, 학생 여러 명 저장, 상담지 인쇄/PDF(학원명·상담자 슬롯). 3D 성적 타워(three.js). 저장은 선택(localStorage) |
| 시험 설계도 | `ipsi/sulgye.html` | 학교알리미 공시 평가계획 파일(hwp·hwpx·pdf·zip)을 브라우저에서 파싱(OLE/CFB + zlib, JSZip, pdf.js) → 과목별 선택형·서답형·수행 비율, 수행 기본점수 제외 실질 변별 점수, 출제 범위, "등급이 갈리는 곳" 순위. 직접 입력·수정 가능, 서버 전송 없음 |
