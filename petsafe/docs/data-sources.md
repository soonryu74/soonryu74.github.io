...
| 동물장묘업 허가 목록 | 농림축산검역본부 국가동물보호정보시스템 반려동물 영업자 정보(동물장묘업) — 공공데이터포털 15121110과 같은 원천 | `npm run import:funeral` 스냅숏(JSON 커밋) | 이용허락범위 제한 없음. 원문 안내: "편의 목적이며 실제 허가·등록 내용과 일부 차이가 있을 수 있음" | 수동(월 1회 권장) | `scripts/import-funeral-businesses.ts`, `src/lib/funeral/parse.ts`, `src/app/funeral` | 2026-09-25 수집 88곳(원문 전체 88건과 일치). 농식품부 장묘업 현황(15073587)은 지역별 업체 수만 있어 업체 확인에는 쓰지 않음 |
