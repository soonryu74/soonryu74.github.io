...
문구·날짜는 `site-config.json`의 `credit` 한 곳에서만 관리합니다. 날짜를 바꾸면
`index.html`·`subscribe.html`은 `credit.js`가 바로 읽고, 발간호와 발간 목록은
`scripts/render_newsletter_issues.py`를 다시 실행하면 반영됩니다. 스타일은 `paper.css`의 `.site-credit` 하나입니다.
