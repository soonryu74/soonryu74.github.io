...
(function () {
  ...
  X.omega3 = {
    ...
    gi: c('ok', '위장 불편 외 문제 없음'),
    cancer: c('ok', 'CYP 상호작용 없음. 악액질에 흔히 사용'),
  ...
  X.probiotic = {
    ...
    autoimmune: c('avoid', '면역억제제 복용·중증 환자: S. boulardii 금기(EMA), 세균 균주도 균혈증 사례 대부분 면역저하자', [ODS + 'Probiotics-HealthProfessional/', 'https://wwwnc.cdc.gov/eid/article/27/8/21-0018_article']),
  ...
  X.coq10 = {
    ...
    pregnancy: c('caution', '임신 안전성 자료 없음'),
  ...
  X.creatine = {
    ...
    stones: c('caution', '수분 섭취 의존. 결석 신호는 없음(이론적)')
  };

  /* ===== 커큐민 ===== */
  X.curcumin = {
    healthy: c('na', '무릎 관절염 외 근거 없음. 고흡수 제형 간 손상 보고', LT + 'NBK548561/'),
    diabetes: c('caution', '혈당 강하 가능, 모니터'),
    ...
    cancer: c('caution', '항산화·CYP 상호작용 이론적. 종양내과 상의'),
    pregnancy: c('avoid', 'NCCIH: 보충제 용량은 임신 중 안전하지 않을 수 있음', NC + 'turmeric'),
    anticoag: c('caution', '혈소판 응집 억제(ex vivo). 임상 의미는 미확인', 'https://onlinelibrary.wiley.com/doi/10.1046/j.1440-6047.2002.00296.x'),
    stones: c('avoid', '강황(수용성 옥살산 91%)은 소변 옥살산을 유의하게 올림(교차 RCT)', 'https://www.researchgate.net/publication/5382784_Effect_of_cinnamon_and_turmeric_on_urinary_oxalate_excretion_plasma_lipids_and_plasma_glucose_in_healthy_subjects'),
    surgery: c('caution', '항혈소판 작용, 수술 1~2주 전 중단', SPAQI)
  };

  /* ===== 콜라겐 ===== */
  X.collagen = {
    healthy: c('ok', '해롭지 않음. 피부 수분 소폭 개선(제조사 지원 시험)'),
    ckd: c('caution', '단백질 10~20 g/일은 신장질환 단백질 제한에 포함. 하이드록시프롤린 → 옥살산'),
    osteo: c('ok', '골밀도 이득 가능(소규모)'),
  ...
  /* ===== 루테인 ===== */
  X.lutein = {
    healthy: c('na', '황반변성 없는 눈 예방 근거 없음'),
  ...
  /* ===== 멜라토닌 ===== */
  X.melatonin = {
    healthy: c('ok', '잠드는 시간 7분 단축. 한국은 전문의약품(처방)'),
  ...
  /* ===== 글루코사민 ===== */
  X.glucosamine = {
    healthy: c('na', 'NIH GAIT: 위약보다 낫지 않음'),
    ...
    anticoag: c('caution', 'FDA MedWatch 20건 + WHO 21건: 와파린 INR 상승·출혈, 용량 의존, 중단 시 회복', 'https://pubmed.ncbi.nlm.nih.gov/18363538/')
  };

  /* ===== 홍국·베르베린 ===== */
  X.ryr = {
    healthy: c('na', 'LDL 관리는 의사와 스타틴 상담. 심혈관 사건 감소 근거 없음'),
    ...
    liver: c('avoid', 'LiverTox B등급(스타틴 유사 간독성). 간경변·활동성 간질환 금기', LT + 'NBK548168/'),
    ...
    cancer: c('avoid', '베르베린 CYP3A4/2D6/P-gp 억제로 치료역 좁은 항암제 농도 변화. 홍국은 CYP3A4 억제제(아졸·일부 TKI)와 노출 증가'),
    pregnancy: c('avoid', '홍국: 스타틴 금기와 동일(NCCIH). 베르베린: 빌리루빈 알부민 결합 치환(페닐부타존 10배) → 핵황달 위험, 태반·모유 통과', [NC + 'red-yeast-rice', 'https://www.ncbi.nlm.nih.gov/books/NBK600384/']),
    anticoag: c('caution', '베르베린 CYP2C9 억제 → 와파린 상승 이론적')
  };

  /* ===== 홍삼 ===== */
  X.ginseng = {
    healthy: c('ok', '피로 소규모 시험 혼재, 질병 예방 근거 없음. 안전성은 양호'),
    htn: c('caution', '메타분석: 홍삼은 혈압 소폭 강하(장기 수축기 −2.9). 과거 상승 보고도 있어 조절 안 되는 고혈압은 주의(식약처 병용정보)', ['https://www.nature.com/articles/jhh201618', 'https://nedrug.mfds.go.kr/bbs/13']),
    diabetes: c('caution', '혈당 강하 가능 → 혈당조절제와 저혈당(NCCIH·식약처)', [NC + 'asian-ginseng', 'https://nedrug.mfds.go.kr/bbs/13']),
    liver: c('ok', 'LiverTox: 단독 간 손상 없음', LT + 'NBK548200/'),
    autoimmune: c('caution', 'NCCIH: 자가면역질환 악화 가능', NC + 'asian-ginseng'),
    cancer: c('caution', '진세노사이드 에스트로겐 유사 작용(in vitro). ER+ 유방암 등 호르몬 의존성 암 피하기. 이마티닙+홍삼 간독성 사례', ['https://pmc.ncbi.nlm.nih.gov/articles/PMC3659583/', LT + 'NBK548200/']),
    pregnancy: c('avoid', 'NCCIH: 안전하지 않을 수 있음(동물 기형 유발 성분)', NC + 'asian-ginseng'),
    anticoag: c('caution', 'Yuan 2004 RCT(미국삼, n=20): 와파린 INR·AUC 감소. 홍삼 교차시험(판막 환자 25명): INR 면밀 감시 하 사용 가능. 식약처: 항응고제 병용 신중', ['https://pubmed.ncbi.nlm.nih.gov/15238367/', 'https://nedrug.mfds.go.kr/bbs/13']),
    surgery: c('caution', 'SPAQI: 수술 2주 전 중단', SPAQI)
  };

  /* ===== 밀크시슬 ===== */
  X.milkthistle = {
    healthy: c('na', '코크란: 간질환 사망·합병증 개선 근거 없음. 안전'),
    ...
    anticoag: c('caution', 'in vitro CYP2C9 억제 → 와파린 INR 모니터(임상 미확인)')
  };

  /* ===== 녹차추출물 ===== */
  X.greentea = {
    healthy: c('na', '체중 감량 효과 미미. 차로 마시는 건 안전, 고용량 추출물은 간독성'),
    ...
    stones: c('caution', '차는 중등도 옥살산 공급원')
  };

  /* ===== 마늘 ===== */
  X.garlic = {
    healthy: c('ok', '혈압 소폭 강하 가능'),
    htn: c('ok', 'NCCIH: 혈압 소폭 강하 가능', NC + 'garlic'),
    dyslip: c('ok', '지질 효과 미미, 상호작용 없음'),
  ...
  /* ===== 은행잎 ===== */
  X.ginkgo = {
    healthy: c('na', '치매 예방 RCT(GEM 3,069명) 실패. 기억력 근거 없음'),
    ...
    cancer: c('caution', '항산화·CYP 이론적'),
    pregnancy: c('avoid', 'NCCIH: 조기 진통·분만 시 출혈 증가 가능', NC + 'ginkgo'),
    anticoag: c('caution', '사례: 와파린+은행잎 뇌출혈 1건, 단독 자발 출혈 ≥4건. RCT(Jiang 2005·Engelsen 2003)와 VA 코호트는 INR 변화 없음. NCCIH는 여전히 출혈 경고', ['https://pubmed.ncbi.nlm.nih.gov/15801937/', 'https://pubmed.ncbi.nlm.nih.gov/26958257/', NC + 'ginkgo']),
    surgery: c('avoid', '수술 36시간~2주 전 중단(SPAQI 2주)', SPAQI)
  };

  /* ===== 세인트존스워트 ===== */
  X.sjw = {
    healthy: c('caution', '경증 우울 효과 있으나 CYP3A4 강력 유도. 복용 중인 모든 약 확인 필수'),
    ...
    hf_af: c('avoid', '디곡신 농도 감소. DOAC(아픽사반·리바록사반)·아미오다론 감소', NC + 'st-johns-wort-and-depression-in-depth'),
  ...
  /* ===== 아슈와간다 ===== */
  X.ashwagandha = {
    healthy: c('caution', '스트레스 소규모 단기 시험만. 간 손상(LiverTox B) 보고. 8주 이상 연속 복용 자제'),
  ...
  X.nac = {
    ...
    pregnancy: c('caution', '치료 목적(아세트아미노펜 중독)으로는 사용. 보충제 용량 안전성 미확인'),
  ...
  X.ala = {
    ...
    autoimmune: c('caution', '인슐린 자가면역 증후군은 자가면역 기전(이론적)'),
    cancer: c('caution', '항암 중 항산화제 우려(JNCI 2008)', 'https://academic.oup.com/jnci/article/100/11/773/895704'),
    pregnancy: c('caution', '자료 불충분')
  };

  /* ===== 크롬 ===== */
  X.chromium = {
    healthy: c('na', '혈당·체중 효과 임상적 의미 없음'),
    ...
    liver: c('caution', '고용량 신손상 사례에서 간 괴사 동반', LT + 'NBK548854/'),
    thyroid: c('caution', '크롬 피콜리네이트 동시 복용 시 레보티록신 AUC 17% 감소(n=7). 수 시간 분리', 'https://pubmed.ncbi.nlm.nih.gov/17725434/'),
    pregnancy: c('ok', '충분섭취량(30 µg) 이내는 문제 없음')
  };

  /* ===== 계피 ===== */
  X.cinnamon = {
    healthy: c('na', '혈당 효과 불확실. 카시아 계피 쿠마린 간독성'),
    ...
    anticoag: c('caution', '카시아 쿠마린은 항응고제가 아니지만 이론적 상가(미확인)'),
    stones: c('ok', '계피는 소변 옥살산을 올리지 않음(교차 RCT)', 'https://www.researchgate.net/publication/5382784_Effect_of_cinnamon_and_turmeric_on_urinary_oxalate_excretion_plasma_lipids_and_plasma_glucose_in_healthy_subjects')
  };
  ...
  X.vitd = {
    ...
    thyroid: c('ok', '갑상선항진 골소실에 칼슘+D 권장. 레보티록신 흡수 문제 없음'),
    ...
    cancer: c('ok', '결핍 시 보충. 림프종·육아종성 종양은 고칼슘혈증 모니터'),
  ...
  X.vita = {
    healthy: c('na', '한국인 결핍 드묾. 고용량 사망률 증가(코크란)'),
  ...
  X.vite = {
    ...
    hf_af: c('caution', 'HOPE-TOO 400 IU: 심부전 RR 1.17', 'https://pubmed.ncbi.nlm.nih.gov/15769967/'),
    ...
    pregnancy: c('caution', '이득 없음. 비타민 C+E 시험에서 임신성 고혈압·조기 양막파열 증가 보고. 권장량 12 mg만'),
    ...
    surgery: c('caution', '항혈소판 작용, 수술 전 중단')
  };
  X.vitc = {
    healthy: c('ok', '감기 예방 못 함. 상한 2,000 mg'),
    ...
    liver: c('avoid', '혈색소증·철 과잉: 500 mg 초과 시 철 흡수 증가(CDC: 피하기). 그 외 간질환은 무방', CDCHH),
    gi: c('ok', '비헴철 흡수 보조. PPI·무산증은 흡수 저하'),
    ...
    pregnancy: c('ok', '권장 85 mg, 상한 2,000 mg'),
    anticoag: c('caution', '1~2 g/일 이상이 와파린 효과 감소(사례 보고, 근거 약함)'),
  ...
  X.vitk = {
    healthy: c('na', '건강인 보충 근거 없음'),
    ckd: c('ok', 'KDOQI 5.5.1: 와파린 아니면 허용', KDOQI),
    liver: c('ok', '담즙정체 결핍. 독성 없음(상한 없음)'),
    hf_af: c('caution', '심방세동 환자 대부분 항응고제 복용 → 아래 항응고제 항목'),
    gi: c('ok', '지방 흡수장애 결핍'),
    pregnancy: c('ok', '문제 없음'),
    anticoag: c('avoid', '와파린 효과 직접 길항. 새 K·K2 보충제 시작 금지, 섭취량 일정 유지. DOAC·아스피린·클로피도그렐은 무관', ['https://ods.od.nih.gov/factsheets/VitaminE-HealthProfessional/', 'https://www.ncbi.nlm.nih.gov/pmc/articles/PMC8395160/'])
  };
  X.vitk2 = X.vitk;
  X.folate = {
    ...
    autoimmune: c('ok', '류마티스 메트렉세이트 독성 감소(표준 병용 처방)'),
    ...
    anticoag: c('ok', '와파린 상호작용 없음')
  };
  X.b12 = {
    healthy: c('ok', '50세+ 흡수 저하로 강화식품·보충제 권고(NIH). 상한 없음'),
    ...
    pregnancy: c('ok', '상한 없음'),
    smoker: c('caution', 'VITAL 코호트: 남성 흡연자 장기 고용량 B12(>55 µg/일) 폐암 약 4배', VITALB)
  };
  X.b6 = {
    healthy: c('na', '100 mg/일 초과 장기 복용 시 감각신경병증(상한 100 mg)', 'https://ods.od.nih.gov/factsheets/VitaminB6-HealthProfessional/'),
    ckd: c('ok', '투석 손실, 소량 보충'),
    pregnancy: c('ok', 'ACOG: 입덧에 10~25 mg 8시간마다. 상한 100 mg'),
    smoker: c('caution', 'VITAL 코호트: 남성 흡연자 장기 >20 mg/일 폐암 약 3배', VITALB),
    stones: c('ok', '소변 옥살산 감소 가능, 해 없음')
  };
  X.biotin = {
    healthy: c('na', '결핍 외 근거 없음. 고용량은 혈액검사 왜곡'),
    dyslip: c('caution', '트로포닌 거짓 저하 → 심근경색 놓침(FDA 2017/2019, 사망 1건 보고)', FDABIO),
    hf_af: c('caution', '트로포닌·NT-proBNP 면역검사 왜곡', FDABIO),
    thyroid: c('caution', 'T4·T3 거짓 상승, TSH 거짓 저하 → 갑상선항진·과량 투약으로 오인. 검사 2일 이상 전 중단', 'https://www.thyroid.org/patient-thyroid-information/ct-for-patients/january-2022/vol-15-issue-1-p-7-8/'),
    autoimmune: c('caution', '자가면역 갑상선 추적검사 왜곡'),
  ...
  X.calcium = {
    healthy: c('na', '지역사회 성인 골절 예방 효과 거의 없음(BMJ 2026). 식사로'),
  ...
  X.magnesium = {
    ...
    autoimmune: c('ok', '타크로리무스·사이클로스포린이 마그네슘 소실 유발'),
    cancer: c('ok', '시스플라틴·세툭시맙 저마그네슘혈증'),
    pregnancy: c('ok', '권장 350~360 mg, 보충 상한 350 mg'),
  ...
  X.zinc = {
    ...
    liver: c('ok', '간경변 결핍 흔함. 40 mg 이하'),
    gi: c('ok', '염증성 장질환 결핍 약 50%. 비만수술 후 아연 8~22 mg + 구리 1~2 mg', ASMBS),
    pregnancy: c('ok', '권장 11 mg, 상한 40 mg')
  };
  X.iron = {
    healthy: c('caution', '검사 없는 복용 비권장. 상한 45 mg'),
    ...
    pregnancy: c('ok', '권장 27 mg(미국)·24 mg(한국), 상한 45 mg')
  };
  X.selenium = {
    healthy: c('na', 'SELECT: 예방 효과 없음'),
    ...
    pregnancy: c('ok', '권장 60 µg, 상한 400 µg')
  };
  X.betacarotene = {
  ...
  X.antioxidant = {
    healthy: c('na', '수명 연장 없음, 베타카로틴·E 사망률 증가'),
  ...
  X.potassium = {
    ...
    diabetes: c('caution', '신장질환 동반·ACEi/ARB 복용 시 고칼륨혈증'),
    ...
    liver: c('caution', '간경변 스피로노락톤 복용 시'),
