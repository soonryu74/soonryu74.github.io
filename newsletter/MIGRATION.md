...
옮길 주소: **`https://epibrief.github.io`**

## 1. 조직 만들기 (무료)

1. <https://github.com/organizations/new> 접속 → **Free** 선택
2. Organization name에 `epibrief` 입력 (이미 쓰는 사람이 있으면 `epibrief-kr` 등으로)
3. 이메일 확인 후 생성

## 2. 저장소 만들기

1. 조직 화면에서 **New repository**
2. 이름을 정확히 **`epibrief.github.io`** 로 (조직명과 같아야 루트 주소가 됩니다)
3. **Public** 선택 → Create

## 3. 파일 올리기

`dist/epibrief/` 폴더의 내용을 **저장소 루트에** 올립니다(폴더째가 아니라 안의 파일들).
...
몇 분 뒤 `https://epibrief.github.io` 에서 열립니다.
...
```json
{ "baseUrl": "https://epibrief.github.io" }
```

## 옮긴 뒤

- 지금 주소(`soonryu74.github.io/newsletter`)는 그대로 둬도 되고, 안내 문구만 남겨 두어도 됩니다.
- 나중에 도메인(예: `epibrief.kr`)을 사면 Settings → Pages → Custom domain 에 넣고
